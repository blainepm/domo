		...
		return {
			...
			controller: ['$scope', 
	            function($scope) {
					...
					var cleanHttpErrorListener = $rootScope.$on('blaineApp.httpError', function (event, httpResponse) {
					    var i;
					        ...
					        case 400:
					            if (httpResponse.data && httpResponse.data.fieldErrors) {
					                for (i = 0; i < httpResponse.data.fieldErrors.length; i++) {
					                    ...
					                    var fieldName = $translate.instant('blaineApp.' + fieldError.objectName + '.' + convertedField);
