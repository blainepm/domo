...
function system()
{
    ...
    echo -e "system"
    echo -e "hostname___$HOSTNAME"
    echo -e "os___$OS $DISTRO"
    echo -e "kernel___$KERNEL_INFO"
    echo -e "uptime___$UPTIME_DAYS day(s), $UPTIME_HOURS hours(s), $UPTIME_MINUTES minute(s)"
    echo -e "last_boot___$LAST_BOOT_DATE $LAST_BOOT_TIME"
    echo -e "current_users___$USERS_NB"
    echo -e "server_datetime___$CURRENT_DATE"
}
...
function load_average()
{
    ...
    echo -e "load"
    echo -e "1_minutes___$LOAD_1"
    echo -e "5_minutes___$LOAD_2"
    echo -e "15_minutes___$LOAD_3"
    echo -e "Processus___$PROCESS_NB process, including $PROCESS_RUN running"
}
...
function cpu()
{
    ...
    if [ $CPU_NB -gt 1 ] ; then
        echo -e "Number___$CPU_NB"
    fi
    echo -e "Model___$CPU_INFO"
    echo -e "Frequency___$CPU_FREQ MHz"
    echo -e "Cache_L2___$CPU_CACHE"
    echo -e "Bogomips___$CPU_BOGOMIPS"
}
...
function network()
{
    ...
    do
        ...
        echo -e "IP_LAN_(${INTERFACE})___$IP_LAN"
    done

    echo -e "IP_WAN___$IP_WAN"
}
